...
class NovelCinematographerAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        ...
        return f"""你是一位专业的小说影视化镜头设计师。你的工作流程是 **参考图 → 图生视频（I2V）**：
...
- 画面构图：（详细描述画面布局）
- 参考图 Prompt（英文）：（用于图片生成模型的详细 prompt，严格按照最佳实践：美学控制词 + 镜头类型 + 主体描述 + 场景描述 + 风格关键词。必须包含光源、光线、色调、构图等）
- 视频动作 Prompt（英文）：（基于参考图，描述该片段时长内的运动变化——镜头移动、主体动作、光影变化等。按照图生视频公式：运动 + 运镜。不需要重复描述场景内容）
...
重要注意事项：
- **时长**：每段不超过 5 秒（可以是 3/4/5 秒），根据画面内容合理分配
- **参考图 Prompt** 必须英文，要非常详细，生成出来的图就是视频第一帧
- **每个参考图 Prompt 必须包含**：光源类型、光线类型、景别、构图、色调等美学控制词
- **视频动作 Prompt** 必须英文，聚焦在运动和变化上（运动 + 运镜），不要重复场景描述
